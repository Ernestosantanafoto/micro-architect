# Estado y progreso – Micro Architect

**Este es el documento para saber en qué punto está el proyecto.**  
Última actualización: 2025-01-31

---

## En una frase

**v0.5-alpha**, Tier 1 (Fase Cuántica). Estabilización en curso; dim/ocultar menú RECURSOS implementado; pendiente testing integral y demo itch.io.

---

## Criterios “T1 funcional” (definition of done)

| Criterio | Estado |
|----------|--------|
| Tech F2 persisten al guardar/cargar | ✅ |
| Save/load con 20+ edificios verificado | ⏳ TEST_CHECKLIST 10.3 |
| Colocación de todos los tipos verificada | ⏳ TEST_CHECKLIST 6.5 |
| TEST_CHECKLIST ejecutado; bugs críticos resueltos | ⏳ |
| ROADMAP 5.1–5.3 marcados al verificar | ⏳ |

---

## Resumen por bloques (ROADMAP)

| Bloque | Estado | Descripción |
|--------|--------|-------------|
| 1–4 | ✅ | Bugs menores, pulido UX, técnico, nucleones |
| **5** | 🔄 En curso | Estabilización v0.5 (save/load, colocación, testing, demo) |
| 6 | ⏳ | Prep técnica Tier 2 (SimulationManager, LOD, velocidad) |
| 7 | ⏳ | Contenido Tier 2 (Electrón, Condensador, Acelerador) |
| 8 | ⏳ | Polish Tier 2 (LOD, blueprints, balance) |

---

## Próximo paso (prioridad)

1. **Estabilizar T1:** Ejecutar TEST_CHECKLIST (6.5 colocación, 10.3 save 20+), corregir bugs críticos.
2. **Demo itch.io:** Export HTML5/Windows, página, descripción, publicar alpha.
3. **Opcional:** Persistir bolas de energía en vuelo (PulseVisual) al cargar/des-dim (ver TASKLIST).

---

## Dónde mirar según lo que necesites

| Si quieres… | Abre |
|-------------|------|
| **Qué hacer ahora** (tareas concretas) | [2_TASKLIST.md](2_TASKLIST.md) |
| Plan detallado por bloques | [4_ROADMAP.md](4_ROADMAP.md) |
| Bugs, versión, sistemas funcionando | [5_PROJECT_STATE.md](5_PROJECT_STATE.md) |
| Pruebas manuales para T1 | [6_TEST_CHECKLIST.md](6_TEST_CHECKLIST.md) |
| Ideas/backlog (opciones, guardado, UX) | [7_COSAS_POR_HACER.md](7_COSAS_POR_HACER.md) |
| Visión 4 tiers y escalas | [8_FUTURE_PLAN.md](8_FUTURE_PLAN.md) |
| Índice de toda la documentación | [3_README.md](3_README.md) |
